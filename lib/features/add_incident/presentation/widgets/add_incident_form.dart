...
class AddIncidentFormState extends ConsumerState<AddIncidentForm> {
                ...
                child: Form(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 10),
                      AddIncidentButton(onPressed: _addIncident),
                    ],
                  ),
                ),
              ),
  ...
  void _listener() {
    ref.listen(addIncidentControllerProvider.select((v) => v.error), (_, next) {
      if (next != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(backgroundColor: Colors.red, content: Text("erreur lors de signaler l'incident")),
        );
      }
    });

    ref.listen(
      addIncidentControllerProvider.select((v) => v.isAddingIncidentSuccess),
      (_, next) {
        if (next == true) {
  ...
  Future<void> _addIncident() async {
    ...
    ref.read(addIncidentControllerProvider.notifier).setFormData(formData);
